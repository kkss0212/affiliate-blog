...
**明日やること(2026-08-30時点のTODO、優先順):**
1. **サイトURL更新**: 楽天アフィリエイトの「メディア管理」画面にログインし、
   登録済みサイトURLが旧URL(`.../japan-unpacked/`)になっていないか確認。
   なっていたら新URL(`https://kkss0212.github.io/`)に更新する。
2. **実リンクを何件か生成**: 手順2・3(下記)の要領で、物販リンク・楽天トラベル
   リンクをいくつか実際に生成してみる。
3. **生成したURLをこちらに共有**: 生成したURLをこのセッション(または新しい
   セッション)に貼ってもらえれば、以下の点数分を一括で記事に埋め込みます。

**⚠️ 埋め込み対象の規模(要注意)**: 現在、47都道府県+19市町村、**合計66記事**に
楽天トラベルの`experiences`エントリがプレースホルダーとして入っています(以前の
メモでは「kyoto.mdの1件のみ」となっていましたが、その後の記事量産で全記事に
広がりました)。**Amazon商品リンクのASINパスとは違い、楽天トラベルは
「タグを1つ差し替えるだけ」ではなく、各記事の場所ごとに実在するホテル/旅館を
検索してリンクを生成する必要がある**ため、66件すべてを埋めるには相応の時間が
かかります。まずは数件(例: 京都・東京など主要な記事)から始めて、残りは
様子を見ながら追加していく進め方を推奨します。
...
- [ ] 楽天アフィリエイト: サイトURLを新URLに更新(メディア管理画面)
- [ ] 楽天アフィリエイト: 実リンクを生成し、対象66記事へ埋め込み(数件ずつ進める想定)
