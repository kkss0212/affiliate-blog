...
- メディア登録後の手順:
  ...
  4. 生成されたURLを、対応する記事の`experiences`(`provider: "rakuten-travel"`)
     または`products`に埋め込む
     — **進捗(2026-08-28): `kyoto.md`が新フォーマットで実内容の下書き
     (`draft: true`、ユーザーレビュー待ち)になり、`rakuten-travel`の
     experienceが1件プレースホルダーとして入った。** レビュー後
     `draft: false`にする際、そのプレースホルダーURLを既に生成済みの
     実リンクに差し替えれば手順4が完了する。他の46都道府県は未着手。
